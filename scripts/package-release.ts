...
rmSync(packageDir, { recursive: true, force: true }); rmSync(questionPack, { force: true }); rmSync(appArchive, { force: true }); mkdirSync(packageDir, { recursive: true });
for (const item of [".next/standalone", ".next/static", "public", "examples", "README.md", "LICENSE", "package.json"]) if (existsSync(item)) cpSync(item, join(packageDir, item.replace(".next/standalone", "app").replace(".next/static", "app/.next/static")), { recursive: true });
cpSync("Dockerfile", join(packageDir, "Dockerfile"));
async function zip(output: string, add: (archive: archiver.Archiver) => void) {
  await new Promise<void>((resolve, reject) => {
    const stream = createWriteStream(output); const archive = archiver("zip", { zlib: { level: 9 } });
